...
export class DataMapper {
  ...
  private _getSizeAsString(sizeBN: BN) {
    ...
    if (sizeString === undefined) {
      sizeString = zetaConvertNativeLotSizeToDecimal(
        this._options.market.baseSizeLotsToNumber(sizeBN).toFixed(this._options.sizeDecimalPlaces)
      );
  ...
  private _mapEventToDataMessage(
    ...
    if (event.eventFlags.fill) {
      const fillMessage: Fill = {
        ...
        size: zetaConvertNativeLotSizeToDecimal(this._getFillSize(event).toFixed(this._options.sizeDecimalPlaces)),
      ...
      const doneMessage: Done = {
        ...
        sizeRemaining:
          reason === 'canceled'
            ? zetaConvertNativeLotSizeToDecimal(this._getDoneSize(event).toFixed(this._options.sizeDecimalPlaces))
            : undefined
      };
      return doneMessage;
    }
...
function zetaConvertNativeLotSizeToDecimal(size: string) {
  return utils.convertNativeLotSizeToDecimal(Number(size)).toString();
}
